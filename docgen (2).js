...
const DocGen = (() => {
  function getD() {
    if (!window.docx) throw new Error('docx.js library failed to load (check your internet connection or the CDN script tag in index.html)');
    return window.docx;
  }

  // Returns { D, cellBorders, cell } scoped to a loaded docx instance.
  function ctx() {
    const D = getD();
  ...
  async function generateBlob(type, meta, components) {
    ...
    return await getD().Packer.toBlob(doc);
  }
